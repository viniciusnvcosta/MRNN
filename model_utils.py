...
class biGRUCell(object):
    ...
    def __init__(self, input_size, hidden_layer_size, target_size):
        ...
        self.Wo = tf.Variable(
            tf.truncated_normal(
                [self.hidden_layer_size * 2, self.target_size], mean=0, stddev=0.01
            )
        )
        self.bo = tf.Variable(
            tf.truncated_normal([self.target_size], mean=0, stddev=0.01)
        )

        # Placeholder for input vector with shape[batch, seq, embeddings]
        self._inputs = tf.placeholder(
            tf.float32, shape=[None, None, self.input_size], name="inputs"
        )
        # Reversing the inputs by sequence for backward pass of the GRU
        self._inputs_rev = tf.placeholder(
            tf.float32, shape=[None, None, self.input_size], name="inputs_rev"
        )
